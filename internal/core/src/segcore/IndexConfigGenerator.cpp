...
VecIndexConfig::VecIndexConfig(const int64_t max_index_row_cout,
    ...
    if (is_sparse && metric_type_ == knowhere::metric::BM25){
        build_params_[knowhere::meta::BM25_K1] = index_meta_.GetIndexParams().at(knowhere::meta::BM25_K1);
        build_params_[knowhere::meta::BM25_B] = index_meta_.GetIndexParams().at(knowhere::meta::BM25_B);
        build_params_[knowhere::meta::BM25_AVGDL] = index_meta_.GetIndexParams().at(knowhere::meta::BM25_AVGDL);
    }

    search_params_[knowhere::indexparam::NPROBE] =
        std::to_string(config_.get_nprobe());

    for (auto it = index_meta_.GetIndexParams().begin(); it != index_meta_.GetIndexParams().end(); ++it){
    ...
    // segment index.
    LOG_INFO(
        "VecIndexConfig: origin_index_type={}, index_type={}, metric_type={}, config={}",
        origin_index_type_,
        index_type_,
        metric_type_,
        build_params_.dump());
...
VecIndexConfig::GetSearchConf(const SearchInfo& searchInfo) {
    ...
    if (metric_type_ == knowhere::metric::BM25){
        searchParam.search_params_[knowhere::meta::BM25_AVGDL] = searchInfo.search_params_[knowhere::meta::BM25_AVGDL];
    }
