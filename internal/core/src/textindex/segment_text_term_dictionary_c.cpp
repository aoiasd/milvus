...
std::vector<const fst_test::TermDictionary*>
TextTermFsts(const CTextFstHandle* fst_handles, int64_t fst_count) {
    ...
    for (int64_t i = 0; i < fst_count; ++i) {
        ...
        fsts.push_back(
            static_cast<const fst_test::burntsushi_fst_cpp_impl::
                            BurntSushiFstCppTermDictionary*>(fst_handles[i]));
    }
    return fsts;
}
...
AddSegmentTextTermFstsToTrie(CSegmentInterface c_segment,
    ...
    try {
        ...
        dictionary = &segment->GetTextTermDictionary();
        const auto fsts = TextTermFsts(fst_handles, fst_count);
...
PrepareSegmentTextTermFuzzyQuery(const uint8_t* query,
    ...
    CTextTermFuzzyQueryBuildResult result{};
    try {
        if (query_size < 0 ||
            ...
            result.status =
                milvus::FailureCStatus(milvus::UnexpectedError,
                                       "invalid segment text term fuzzy query");
            return result;
        }
        ...
        auto prepared =
            fst_test::burntsushi_fst_cpp_impl::PrepareLevenshteinQuery(
                query_view,
                max_edit_distance,
                true,
                prefix_length,
                static_cast<size_t>(std::min<uint64_t>(
                    work_budget, std::numeric_limits<size_t>::max())));
        result.work_used = static_cast<uint64_t>(prepared.work_used);
        result.work_limit_exceeded = prepared.work_limit_exceeded;
        if (!prepared.work_limit_exceeded) {
            result.handle =
                new fst_test::burntsushi_fst_cpp_impl::PreparedLevenshteinQuery(
                    std::move(*prepared.query));
        }
        result.status = milvus::SuccessCStatus();
    } catch (const std::bad_alloc& e) {
        delete static_cast<
            fst_test::burntsushi_fst_cpp_impl::PreparedLevenshteinQuery*>(
            result.handle);
        ...
        result.status = milvus::FailureCStatus(
            milvus::UnexpectedError,
            "unknown segment text term fuzzy query preparation failure");
    }
    return result;
}
...
DeleteSegmentTextTermFuzzyQuery(CTextTermFuzzyQueryHandle handle) {
    delete static_cast<
        fst_test::burntsushi_fst_cpp_impl::PreparedLevenshteinQuery*>(handle);
}
...
FuzzySearchSegmentTextTermsPrepared(CSegmentInterface c_segment,
    ...
    try {
        if (c_segment == nullptr || fst_count < 0 ||
            (fst_count > 0 && fst_handles == nullptr) || query == nullptr ||
            max_expansions == 0) {
            result.status = milvus::FailureCStatus(
                milvus::UnexpectedError,
                "invalid prepared segment text term fuzzy request");
            return result;
        }
        auto* segment =
            static_cast<milvus::segcore::SegmentInternalInterface*>(c_segment);
        const auto fsts = TextTermFsts(fst_handles, fst_count);
        const auto fuzzy = segment->GetTextTermDictionary().FuzzySearchPrepared(
            field_id,
            fsts,
            *static_cast<const fst_test::burntsushi_fst_cpp_impl::
                             PreparedLevenshteinQuery*>(query),
            max_expansions,
            static_cast<size_t>(std::min<uint64_t>(
                work_budget, std::numeric_limits<size_t>::max())));
        result.work_used = static_cast<uint64_t>(fuzzy.work_used);
        result.work_limit_exceeded = fuzzy.work_limit_exceeded;
        if (!fuzzy.work_limit_exceeded) {
            SetTextTermFuzzyMatches(&result, fuzzy.matches);
        }
        result.status = milvus::SuccessCStatus();
    } catch (const std::bad_alloc& e) {
        ...
        result.status = milvus::FailureCStatus(
            milvus::UnexpectedError,
            "unknown prepared segment text term fuzzy search failure");
    }
    return result;
}
...
FuzzySearchSegmentTextTerms(CSegmentInterface c_segment,
    ...
    try {
        ...
        const std::string_view query_view(
            query_size == 0 ? "" : reinterpret_cast<const char*>(query),
            static_cast<size_t>(query_size));
        const auto fuzzy = segment->GetTextTermDictionary().FuzzySearch(
            field_id,
            fsts,
            query_view,
            max_edit_distance,
