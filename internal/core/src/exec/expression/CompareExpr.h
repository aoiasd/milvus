...
class PhyCompareFilterExpr : public Expr {
 ...
 private:
    ...
    ProcessBothDataChunks(FUNC func,
        ...
        const auto processed_size =
            TryProcessBothDataByScan<T, U, FUNC, ValTypes...>(
                func, GetNextBatchSize(), res, valid_res, values...);
        if (processed_size >= 0) {
    ...
    ProcessBothSortedDataByOffsets(FUNC func,
        ...
        options.predicate = ChunkedColumnInterface::ScanPredicate::None;
        options.start_offset = scan_start;
        options.length = scan_length;
        options.max_batch_rows = batch_size_;
        options.projection = ChunkedColumnInterface::ScanProjection::Data;

        options.value_kind = DataScanValueKind<T>();
        auto left_cursor = left_column->Scan(op_ctx_, options).data_cursor;
        options.value_kind = DataScanValueKind<U>();
        auto right_cursor = right_column->Scan(op_ctx_, options).data_cursor;
        AssertInfo(left_cursor != nullptr && right_cursor != nullptr,
                   "compare offset scan cursor is null");

        ChunkedColumnInterface::DataScanBatch left_batch;
        ChunkedColumnInterface::DataScanBatch right_batch;
    ...
    TryProcessBothDataByScan(FUNC func,
        ...
        if (!data_scan_initialized_) {
            ...
            if (left_column != nullptr && right_column != nullptr) {
                ...
                options.value_kind = DataScanValueKind<T>();
                left_data_cursor_ =
                    left_column->Scan(op_ctx_, options).data_cursor;
                options.value_kind = DataScanValueKind<U>();
                right_data_cursor_ =
                    right_column->Scan(op_ctx_, options).data_cursor;
            }
        }
 ...
 private:
    ...
    std::unique_ptr<ChunkedColumnInterface::DataScanCursor> left_data_cursor_{
        nullptr};
    std::unique_ptr<ChunkedColumnInterface::DataScanCursor> right_data_cursor_{
        nullptr};
    ChunkedColumnInterface::DataScanBatch left_data_batch_;
    ChunkedColumnInterface::DataScanBatch right_data_batch_;
    int64_t left_data_batch_pos_{0};
    int64_t right_data_batch_pos_{0};

    bool
    EnsureDataScanBatch(
        std::unique_ptr<ChunkedColumnInterface::DataScanCursor>& cursor,
