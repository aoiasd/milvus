...
class FixedWidthRowIdScanCursor final
 ...
 public:
    ...
    Next(ChunkedColumnInterface::RowIdScanBatch* out) override {
        ...
        out->row_ids.reserve(kOutputBatchRows);
        while (scan_pos_ < scan_end_ &&
               out->row_ids.size() < kOutputBatchRows) {
            ...
            ScanSpan(span.get(), rows_to_scan, out);

            scan_pos_ += rows_to_scan;
            current_chunk_offset_ += rows_to_scan;
        }
        return !out->row_ids.empty() || scan_pos_ < scan_end_;
    }

 private:
    static constexpr size_t kOutputBatchRows = 8192;
    ...
    ScanTypedSpan(const SpanBase& span,
                  ...
                  ChunkedColumnInterface::RowIdScanBatch* out) {
        T value{};
        ...
        for (int64_t i = 0; i < rows_to_scan; ++i) {
            ...
            if (UnaryScanCompare<T>(data[local_offset], value, op_type_)) {
                out->row_ids.emplace_back(scan_pos_ + i);
            }
        }
    }
    ...
    ScanSpan(const SpanBase& span,
        ...
        switch (data_type_) {
            case DataType::BOOL:
                ScanTypedSpan<bool>(span, rows_to_scan, out);
                break;
            case DataType::INT8:
                ScanTypedSpan<int8_t>(span, rows_to_scan, out);
                break;
            case DataType::INT16:
                ScanTypedSpan<int16_t>(span, rows_to_scan, out);
                break;
            case DataType::INT32:
                ScanTypedSpan<int32_t>(span, rows_to_scan, out);
                break;
            case DataType::INT64:
            case DataType::TIMESTAMPTZ:
                ScanTypedSpan<int64_t>(span, rows_to_scan, out);
                break;
            case DataType::FLOAT:
                ScanTypedSpan<float>(span, rows_to_scan, out);
                break;
            case DataType::DOUBLE:
                ScanTypedSpan<double>(span, rows_to_scan, out);
                break;
            default:
                break;
        }
...
class ViewDataScanCursor final : public ChunkedColumnInterface::DataScanCursor {
 public:
...
class IntersectRowIdScanCursor final
    : public ChunkedColumnInterface::RowIdScanCursor {
 public:
    IntersectRowIdScanCursor(
        std::unique_ptr<ChunkedColumnInterface::RowIdScanCursor> left,
        std::unique_ptr<ChunkedColumnInterface::RowIdScanCursor> right)
    ...
    Next(ChunkedColumnInterface::RowIdScanBatch* out) override {
        AssertInfo(out != nullptr,
                   "intersect row id scan output batch is null");
        out->row_ids.clear();
        out->row_ids.reserve(kOutputBatchRows);

        while (out->row_ids.size() < kOutputBatchRows) {
            if (!EnsureLeft() || !EnsureRight()) {
                return !out->row_ids.empty();
            }
            const auto left_row = left_batch_.row_ids[left_pos_];
            const auto right_row = right_batch_.row_ids[right_pos_];
            if (left_row == right_row) {
                out->row_ids.emplace_back(left_row);
                ++left_pos_;
                ++right_pos_;
 ...
 private:
    ...
    bool
    EnsureLeft() {
        while (left_pos_ >= left_batch_.row_ids.size()) {
            left_pos_ = 0;
            if (!left_->Next(&left_batch_)) {
                return false;
            }
        }
        return true;
    ...
    bool
    EnsureRight() {
        while (right_pos_ >= right_batch_.row_ids.size()) {
            right_pos_ = 0;
            if (!right_->Next(&right_batch_)) {
                return false;
            }
        }
        return true;
    ...
    std::unique_ptr<ChunkedColumnInterface::RowIdScanCursor> left_;
    std::unique_ptr<ChunkedColumnInterface::RowIdScanCursor> right_;
    ChunkedColumnInterface::RowIdScanBatch left_batch_;
    ChunkedColumnInterface::RowIdScanBatch right_batch_;
    size_t left_pos_{0};
    size_t right_pos_{0};
};
...
inline std::unique_ptr<ChunkedColumnInterface::RowIdScanCursor>
...
inline std::unique_ptr<ChunkedColumnInterface::DataScanCursor>
...
ChunkedColumnInterface::Scan(milvus::OpContext* op_ctx,
    ...
    if (options.output == ScanOutput::Data) {
        ...
        result.data_cursor = detail::MakeDataScanCursor(this,
                                                        op_ctx,
    ...
    switch (options.predicate) {
        case ScanPredicate::Unary:
            result.row_id_cursor =
                detail::MakeFixedWidthRowIdScanCursor(this,
                                                      op_ctx,
                                                      options.start_offset,
                                                      options.length,
                                                      *data_type,
        ...
        case ScanPredicate::BinaryRange:
            result.row_id_cursor =
                detail::MakeFixedWidthBinaryRangeRowIdScanCursor(
                    this,
                    op_ctx,
                    options.start_offset,
                    options.length,
