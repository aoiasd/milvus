...
func CreateSearchScorer(schema *typeutil.SchemaHelper, function *schemapb.FunctionSchema, exprTemplateValues map[string]*schemapb.TemplateValue) (*planpb.ScoreFunction, error) {
	rerankerName := GetRerankName(function)
	switch rerankerName {
	case "boost":
		scorer := &planpb.ScoreFunction{}
		filter, ok := funcutil.TryGetAttrByKeyFromRepeatedKV("filter", function.GetParams())
		if ok {
		...
		weightStr, ok := funcutil.TryGetAttrByKeyFromRepeatedKV("weight", function.GetParams())
		if !ok {
