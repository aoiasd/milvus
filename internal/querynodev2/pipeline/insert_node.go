...
func (iNode *insertNode) fillEmbeddingData(schema *schemapb.CollectionSchema, msg *InsertMsg) error {
	_, ok, err := function.FillCachedFunctionData(iNode.collectionID, msg.InsertRequest)
	if err != nil {
		return err
	}
	if ok {
...
func (iNode *insertNode) initEmbeddingOutputFieldIDs(schema *schemapb.CollectionSchema) ([]int64, error) {
	outputFieldIDs, err := function.EmbeddingOutputFieldIDs(schema)
	if err != nil {
		return nil, err
	}
	iNode.functionOutputFieldIDs = outputFieldIDs
...
func (iNode *insertNode) Close() {
	...
	iNode.functionOutputFieldIDs = nil
}
...
func (iNode *insertNode) Operate(in Msg) Msg {
	...
	if len(nodeMsg.insertMsgs) > 0 {
		...
		for _, msg := range nodeMsg.insertMsgs {
			if len(iNode.functionOutputFieldIDs) > 0 && !function.HasAllFieldDataByID(msg.GetFieldsData(), iNode.functionOutputFieldIDs) {
...
func newInsertNode(
	...
	if _, err := iNode.initEmbeddingOutputFieldIDs(schema); err != nil {
		return nil, err
	}
	return iNode, nil
}
...
func getBM25OutputFieldIDs(schema *schemapb.CollectionSchema) ([]int64, error) {
	...
	for _, fn := range schema.GetFunctions() {
		...
		default:
			return nil, fmt.Errorf("unknown function type %s", fn.GetType().String())
		}
	}
	return outputFieldIDs, nil
}
