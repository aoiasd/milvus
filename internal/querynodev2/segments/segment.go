...
func (s *LocalSegment) ExpandTextTerms(
	...
	workBudget uint64,
) ([][]TextTermMatch, uint64, int32, uint64, error) {
	s.lexicalMu.RLock()
	defer s.lexicalMu.RUnlock()
	if !s.ptrLock.PinIf(state.IsNotReleased) {
		return nil, 0, 0, 0, merr.WrapErrSegmentNotLoaded(s.ID(), "segment released")
	}
	defer s.ptrLock.Unpin()

	matches, work, err := s.textTerms.expandPreparedWithinBudget(
		fieldID, prepared, maxExpansions, workBudget)
	if err != nil {
		return nil, 0, 0, work, err
	}
	return matches, s.currentTextTermGeneration(), s.LoadInfo().GetDataVersion(), work, nil
}
