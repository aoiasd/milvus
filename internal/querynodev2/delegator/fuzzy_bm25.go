...
type FuzzyTextTermExpander interface {
	ExpandTextTerms(
		...
		workBudget uint64,
	) ([][]textindex.FuzzyMatch, uint64, uint64, error)
}
...
// ExpandFuzzyBM25Terms runs the resource-controlled expansion shared by local
// Delegator targets and sealed Worker targets.
func ExpandFuzzyBM25Terms(
	...
	remainingWork := min(
		request.GetWorkBudget(),
		paramtable.Get().QueryNodeCfg.FuzzyExpansionMaxWork.GetAsUint64(),
	)
	if remainingWork == 0 {
	...
	prepared, preparationWork, err := textindex.PrepareFuzzySearchTerms(
		request.GetSourceTerms(), request.GetMaxEditDistance(), request.GetPrefixLength(), remainingWork)
	response.WorkUsed += preparationWork
	if preparationWork > remainingWork {
		return fail(merr.WrapErrServiceInternalMsg("fuzzy query preparation exceeded its work budget"))
	}
	...
	for _, target := range targets {
		...
		matches, generation, work, err := target.Expander.ExpandTextTerms(
			request.GetFieldID(), prepared, request.GetMaxExpansions(), remainingWork)
		response.WorkUsed += work
		if err != nil {
			return fail(err)
		}
		if work > remainingWork {
			return fail(merr.WrapErrServiceInternalMsg("text term expansion exceeded its work budget"))
		}
		remainingWork -= work
