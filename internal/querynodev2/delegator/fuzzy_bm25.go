...
func fuzzyBM25ExpansionWorkLimitError(maxWork uint64) error {
	status := merr.Status(merr.Wrapf(
		merr.ErrServiceResourceInsufficient,
		"fuzzy BM25 term expansion exhausted queryNode.fuzzyExpansion.maxWork %d", maxWork))
	status.Retriable = false
	return textindex.MarkFuzzySearchWorkLimitExceeded(merr.Error(status))
}
...
func (sd *shardDelegator) expandLocalFuzzyBM25Terms(
	...
	remainingWork := request.GetWorkBudget()
	if remainingWork == 0 {
		return nil, merr.WrapErrServiceInternalMsg("local text term expansion requires a work budget")
	}
	local := make([]SegmentEntry, 0, len(growing)+len(sealed))
	local = append(local, growing...)
	local = append(local, sealed...)
	expanders := make([]fuzzyTextTermExpander, len(local))
	...
	if !sd.fuzzyExpansionNativeSemaphore.TryAcquire() {
		return fail(merr.WrapErrTooManyRequests(
			int32(sd.fuzzyExpansionNativeSemaphore.Cap()), "fuzzy BM25 native expansion concurrency is saturated"))
	}
	...
	sd.fuzzyExpansionNativeSemaphore.Release()
	response.WorkUsed += preparationWork
	if preparationWork > remainingWork {
		return fail(merr.WrapErrServiceInternalMsg("local fuzzy query preparation exceeded its work budget"))
	}
	...
	for index, entry := range local {
		...
		matches, generation, dataVersion, work, err := expander.ExpandTextTerms(
			request.GetFieldID(),
			prepared,
		...
		if work > remainingWork {
			return fail(merr.WrapErrServiceInternalMsg("local text term expansion exceeded its work budget"))
		}
		remainingWork -= work
...
func (sd *shardDelegator) expandFuzzyBM25Terms(
	...
	maxWork := paramtable.Get().QueryNodeCfg.FuzzyExpansionMaxWork.GetAsUint64()
	request := &querypb.ExpandTextTermsRequest{
	...
	ambiguousWorkAccounting := false
	successSegments := typeutil.NewSet[int64]()
	failureSegments := make([]int64, 0)
	var expansionErrors []error
	served := make(map[int64]fuzzySegmentGeneration, totalSegmentCount)
	candidates := make(map[uint32]map[string]*querypb.ExpandedTextTerm)
	...
	mergeResponse := func(segmentIDs []int64, response *querypb.ExpandTextTermsResponse) error {
		...
		responseGenerations := make(map[int64]fuzzySegmentGeneration, len(segmentIDs))
		...
		for segmentID, current := range responseGenerations {
			...
			if _, ok := served[segmentID]; !ok {
				nextSize := aggregateWireSize + fuzzyBM25GenerationWireSize(&querypb.SegmentTextTermGeneration{
					SegmentID: segmentID, Generation: current.generation, DataVersion: current.dataVersion,
				})
	...
	record := func(segments []int64, response *querypb.ExpandTextTermsResponse, err error) error {
		if response != nil {
			work := response.GetWorkUsed()
			if work > remainingWork {
				return merr.WrapErrServiceInternalMsg("text term expansion exceeded its delegated work budget")
			}
			remainingWork -= work
			if response.GetWorkLimitExceeded() {
				err = fuzzyBM25ExpansionWorkLimitError(maxWork)
			}
		}
		if err != nil {
	...
	if len(localIDs) > 0 {
		request.WorkBudget = remainingWork
	...
	for index, task := range tasks {
		if remainingWork == 0 {
			for _, pending := range tasks[index:] {
				failureSegments = append(failureSegments, pending.req.GetSegmentIDs()...)
			}
			if ambiguousWorkAccounting {
				break
			}
			err := fuzzyBM25ExpansionWorkLimitError(maxWork)
			expansionErrors = append(expansionErrors, err)
		...
		task.req.WorkBudget = remainingWork
		...
		default:
			delegatedWork := task.req.GetWorkBudget()
			// The Worker charges native traversal against this request's shared
			// budget. Retrying an ambiguously completed RPC would spend it again.
			workerCtx := retry.WithMaxAttemptsContext(ctx, 1)
			response, expandErr = task.worker.ExpandTextTerms(workerCtx, task.req)
			sd.fuzzyExpansionRPCSemaphore.Release()
			if response == nil && expandErr != nil {
				response = &querypb.ExpandTextTermsResponse{WorkUsed: delegatedWork}
				ambiguousWorkAccounting = true
			}
	...
	if len(expansionErrors) > 0 {
		// merr.Combine projects the code of the last error in the chain. Keep a
		// deterministic work-limit failure authoritative over incidental Worker
		// failures collected later by partial-result evaluation.
		for i, err := range expansionErrors {
			if errors.Is(err, textindex.ErrFuzzySearchWorkLimitExceeded) {
				copy(expansionErrors[i:], expansionErrors[i+1:])
				expansionErrors[len(expansionErrors)-1] = err
				break
			}
		}
	...
	for attempt := 1; attempt <= 2; attempt++ {
		...
		if err != nil {
			...
			if merr.IsRetryableErr(err) &&
				!errors.Is(err, textindex.ErrFuzzySearchWorkLimitExceeded) && attempt < 2 {
