...
func (state *collectionState) setRLSPrincipalTagsSnapshot(snapshot principalTagsSnapshot) bool {
	...
	state.principalTags = map[string]map[string]rlsutil.TagValue{}
...
func (state *collectionState) getCompiledExprAndTags(principalName string, action rlsutil.PolicyAction, kind exprKind, schemaHelper *typeutil.SchemaHelper, _ *planparserv2.ParserVisitorArgs, exprSelector func(*rlsutil.RowPolicy) string) (*compiledExpression, map[string]rlsutil.TagValue, error) {
	key := compiledKey{
		action: action,
		kind:   kind,
	}
