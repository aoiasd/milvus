...
// handleIfSearchByPK combines preflight and transformation for callers that
// already own a mutable request.
func (node *Proxy) handleIfSearchByPK(ctx context.Context, request *milvuspb.SearchRequest) ([]bool, *planpb.Expr, error) {
	preflight, err := node.preflightSearchByPK(ctx, request)
	if err != nil {
		return nil, nil, err
	}
