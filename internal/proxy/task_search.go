...
func (t *searchTask) initAdvancedSearchRequest(ctx context.Context) error {
	...
	for index, subReq := range t.request.GetSubReqs() {
		plan, queryInfo, _, offset, err := t.tryGeneratePlan(subReq.GetSearchParams(), subReq.GetDsl(), true)
		if err != nil {
			return err
		}
...
func (t *searchTask) initSearchRequest(ctx context.Context) error {
	...
	plan, queryInfo, field, offset, err := t.tryGeneratePlan(t.request.GetSearchParams(), t.request.GetDsl(), false)
	if err != nil {
		return err
	}
...
func (t *searchTask) tryGeneratePlan(params []*commonpb.KeyValuePair, dsl string, ignoreOffset bool) (*planpb.PlanNode, *planpb.QueryInfo, *schemapb.FieldSchema, int64, error) {
