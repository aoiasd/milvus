...
func (t *searchTask) searchShard(ctx context.Context, nodeID int64, qn types.QueryNodeClient, channel string) error {
	...
	if result.GetStatus().GetErrorCode() != commonpb.ErrorCode_Success {
		...
		if result.GetStatus().GetExtraInfo()[textindex.FuzzySearchWorkLimitExceededFlagKey] == "true" {
			searchErr = retry.Unrecoverable(textindex.MarkFuzzySearchWorkLimitExceeded(searchErr))
		}
