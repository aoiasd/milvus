...
// NewQueryTask constructs a query task. The retrieve request carries the
// per-invocation differences (partition IDs, consistency level, query label);
// everything derived from the host node comes from the taskmodel.TaskNode
// contract and paramtable. cache is the meta cache the task reads from, and
// mustUsePartitionKey carries the proxy.mustUsePartitionKey policy. The caller
// decides its value: external Query requests pass the configured flag, while
// internal reads (e.g. the upsert requery) pass false to keep the previous
...
func NewQueryTask(ctx context.Context, node taskmodel.TaskNode, request *milvuspb.QueryRequest, plan *planpb.PlanNode, retrieveReq *internalpb.RetrieveRequest, cache metacache.Cache, mustUsePartitionKey bool) *QueryTask {
	return &QueryTask{
		...
		mustUsePartitionKey: mustUsePartitionKey,
	}
}
...
// reads, independent of the channelsMvcc input overrides.
func (t *QueryTask) SetActualChannelsMvcc(channelReadTs *typeutil.ConcurrentMap[string, uint64]) {
	t.actualChannelsMvcc = channelReadTs
}

// RecordChannelMvcc records the MVCC timestamp observed on a channel during a
// shard read. It is a no-op when the output snapshot has not been installed.
func (t *QueryTask) RecordChannelMvcc(channel string, ts uint64) {
	if t.actualChannelsMvcc != nil {
		t.actualChannelsMvcc.Insert(channel, ts)
	}
}

// HasRecordedChannelMvcc reports whether an output MVCC snapshot is installed.
func (t *QueryTask) HasRecordedChannelMvcc() bool {
	return t.actualChannelsMvcc != nil
}
...
// Plan returns the query plan owned by this task.
func (t *QueryTask) Plan() *planpb.PlanNode {
	return t.plan
}
...
// SkipRuntimeRLS reports whether runtime RLS resolution is disabled.
func (t *QueryTask) SkipRuntimeRLS() bool {
	return t.skipRuntimeRLS
}
...
// PreserveRawFields reports whether result field conversion is disabled.
func (t *QueryTask) PreserveRawFields() bool {
	return t.preserveRawFields
}

// StorageCost returns the storage cost reported by the last execution.
func (t *QueryTask) StorageCost() segcore.StorageCost {
	return t.storageCost
}

// ReQuery reports whether this task was built as a requery of an earlier search.
func (t *QueryTask) ReQuery() bool {
	return t.reQuery
}

// AllQueryCnt returns the total number of queries counted across shards.
func (t *QueryTask) AllQueryCnt() int64 {
	return t.allQueryCnt
}

// TotalRelatedDataSize returns the accumulated related data size.
func (t *QueryTask) TotalRelatedDataSize() int64 {
	return t.totalRelatedDataSize
}

// GetMetricLabel returns the metric label of this query.
func (t *QueryTask) GetMetricLabel() string {
