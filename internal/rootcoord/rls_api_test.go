...
func TestCoreRejectsOversizedRLSRepeatedFieldsBeforeConversion(t *testing.T) {
	core := newTestCore()
	actions := make([]milvuspb.RowPolicyAction, 9)

	status, err := core.CreateRowPolicy(context.Background(), &milvuspb.CreateRowPolicyRequest{Actions: actions})
	require.NoError(t, err)
	require.ErrorIs(t, merr.Error(status), merr.ErrParameterInvalid)

	status, err = core.UpdateRowPolicy(context.Background(), &milvuspb.UpdateRowPolicyRequest{Actions: actions})
	require.NoError(t, err)
	require.ErrorIs(t, merr.Error(status), merr.ErrParameterInvalid)

	status, err = core.DeleteRLSPrincipalTags(context.Background(), &milvuspb.DeleteRLSPrincipalTagsRequest{
		TagKeys: make([]string, rlsutil.MaxTransportTagKeys+1),
	})
	require.NoError(t, err)
	require.ErrorIs(t, merr.Error(status), merr.ErrParameterTooLarge)
}
...
func TestCoreListRLSPrincipalsPreservesQuotaStatus(t *testing.T) {
	meta := mockrootcoord.NewIMetaTable(t)
	meta.EXPECT().ListRLSPrincipals(mock.Anything, mock.MatchedBy(func(req *rlsutil.ListRLSPrincipalsRequest) bool {
		return req.GetDbName() == "db" && req.GetCollectionName() == "coll"
	})).Return(nil, merr.WrapErrServiceQuotaExceededMsg("list too large")).Once()

	core := newTestCore(withHealthyCode(), withMeta(meta))
	resp, err := core.ListRLSPrincipals(context.Background(), &milvuspb.ListRLSPrincipalsRequest{
		DbName:         "db",
		CollectionName: "coll",
	})
	require.NoError(t, err)
